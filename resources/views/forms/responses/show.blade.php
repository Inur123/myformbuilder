            ...
            <div class="bg-white rounded-lg shadow-sm p-4 border border-indigo-500">
                <div class="flex justify-between items-center">
                    ...
                    <div class="flex items-center space-x-2 text-gray-700">
                        ...
                        {{-- Tombol Previous --}}
                        @if ($prevResponse)
                            <a href="{{ route('forms.responses.show', [$form, $prevResponse]) }}"
                        ...
                        <span class="font-semibold">{{ $currentResponseIndex }}</span> dari <span
                            class="text-gray-500">{{ $totalResponses }}</span>

                        {{-- Tombol Next --}}
                        @if ($nextResponse)
                            <a href="{{ route('forms.responses.show', [$form, $nextResponse]) }}"
                                class="text-gray-500 hover:text-indigo-600">
                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M9 5l7 7-7 7"></path>
                    ...
                    <div class="flex items-center space-x-4">
                        <button class="text-gray-500 hover:text-red-600" title="Hapus Jawaban"
                            onclick="document.getElementById('delete-response-form').submit()">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <form id="delete-response-form" method="POST"
                            action="{{ route('forms.responses.destroy', [$form, $response]) }}" style="display:none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    </div>
                            ...
                            @elseif ($field['type'] === 'file' && $answer !== '-')
                                ...
                                <a href="{{ Storage::url($answer) }}" target="_blank"
                                    ...
                                    <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4-4m0 0L8 12m4-4v12"></path>
                                    </svg>
