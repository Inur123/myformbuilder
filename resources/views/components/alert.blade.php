...
<div x-data="{ show: true, timeout: null }" x-init="timeout = setTimeout(() => { show = false }, 2500)" x-show="show"
    x-transition:enter="transform ease-out duration-300 transition"
    x-transition:enter-start="translate-y-2 opacity-0 sm:translate-y-0 sm:translate-x-2"
    x-transition:enter-end="translate-y-0 opacity-100 sm:translate-x-0"
    x-transition:leave="transition ease-in duration-100" x-transition:leave-start="opacity-100"
    x-transition:leave-end="opacity-0" class="{{ $wrapperClasses }}" style="z-index: 50;">
    <div class="p-4 flex items-start">
        <div class="flex-shrink-0">
            @if ($isSuccess)
                <svg class="{{ $iconClasses }}" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                    stroke="currentColor">
        ...
        <div class="ml-4 flex-shrink-0 flex">
            <button @click="show = false; clearTimeout(timeout)" type="button"
